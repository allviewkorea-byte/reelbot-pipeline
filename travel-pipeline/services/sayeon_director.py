...
def _build_director_prompt(n: int) -> str:
    return (
        ...
        "- ⚠️ emotion 은 곰 표정의 핵심 재료다. protagonist/two_shot 컷마다 비트 감정에 "
        "맞는 **뚜렷한 emotion** 을 고른다(밋밋하게 전부 deadpan 으로 두지 말 것). "
        "정말 무표정이 어울리는 비트에만 deadpan. 마지막 질문 엔딩 컷은 진지하게 묻는 "
        "표정(예: anxiety 또는 deadpan 의 정면 응시)으로.\n"
