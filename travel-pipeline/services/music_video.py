...
def _genre_image_prompt(theme: dict, *, include_prefix: bool = False) -> str | None:
    """주제의 장르를 분류해 LLM 이미지 프롬프트 1개 생성(없으면 None=폴백).

    include_prefix=False(기본): 복사용 — 프리픽스 제외(대표 ChatGPT 스킬 고정).
    """
    return music_image_prompts.get_image_prompt(
        music_genres.classify_theme(theme), include_prefix=include_prefix
    )
...
def generate_background(theme: dict, out_path: str, *, force: bool = False) -> str:
    """배경 1장 → out_path. #작업지시서 2026-06: gpt-image-1 호출 제거 → **블랙 단색**(비용 0).

    대표가 깨끗한 이미지를 업로드하면 make_video(background_path=...)로 그 이미지가 쓰이고,
    미업로드 시(자동) 여기서 블랙 배경을 만든다. R2 멱등은 불필요(블랙은 즉시 생성).
    """
    out = Path(out_path)
    out.parent.mkdir(parents=True, exist_ok=True)
    # ffmpeg 로 1프레임 블랙 PNG 생성(PIL 의존성 없이, ffmpeg 는 이미 필수).
    subprocess.run(
        ["ffmpeg", "-y", "-f", "lavfi", "-i", f"color=c=black:s={W}x{H}",
         "-frames:v", "1", os.path.abspath(str(out))],
        check=True, capture_output=True, text=True,
    )
    logger.info("[video] 블랙 배경 생성(gpt-image-1 미사용)")
    return str(out)
