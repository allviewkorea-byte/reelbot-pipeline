...
# 썸네일에서 사람·갈색곰 등장을 막는 추가 NEGATIVE(부록 C 의 SAYEON_NEGATIVE 위에 덧댄다).
# 썸네일 주인공은 항상 흰곰이므로 갈색곰을 명시적으로 배제한다.
THUMBNAIL_HUMAN_NEGATIVE = (
    "realistic human, real person, child, boy, girl, man, woman, "
    "human face, human figure, person in background, "
    "brown bear, dark brown bear"
)
...
def build_thumbnail_prompt(anchor: str, tone: str, emotion: str) -> str:
    ...
    # [NEGATIVE — 고정] + 곰 둘 금지 + 사람·갈색곰 등장 금지.
...
def generate_thumbnail(
    ...
    생성한다(t2i 금지, 사람/갈색곰 등장 불가). 생성/품질게이트 실패 시 image_url(기존
    씬 컷)로 폴백하고 thumbnail_fallback=True 를 돌려준다(파이프라인은 멈추지 않음).
