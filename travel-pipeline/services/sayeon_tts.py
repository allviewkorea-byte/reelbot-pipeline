...
# (옵션) 라인 간 음량 평탄화 — SAYEON_TTS_VOLUME_MODE="even" 일 때 concat 후 1회 적용.
# 시간축 동적 정규화라 길이는 불변(scene_timings 영향 없음). 짧은 엔딩 라인도 균등해짐.
_DYNAUDNORM = "dynaudnorm=f=200:g=15"


def _to_wav(src: Path, dst: Path, speed: float = 1.0) -> None:
    """입력 오디오를 공통 규격(pcm_s16le 44.1kHz mono) + 타깃 라우드니스로 정규화.

    speed != 1.0 이면 atempo 로 라인 속도 조절(피치 보존). ★라인 단계 적용이라
    이후 _duration 이 변경된 클립을 측정 → scene_timings 가 자동 정합된다.
    ...
    if speed and abs(speed - 1.0) > 1e-3:
        af = f"{_LOUDNORM},atempo={speed:.3f}"  # atempo 단일단 0.5~2.0 유효
    _run(["ffmpeg", "-y", "-i", str(src), "-af", af,
          "-ar", "44100", "-ac", "1", "-c:a", "pcm_s16le", str(dst)])


def _even_loudness(src: Path, dst: Path) -> None:
    """concat 된 나레이션 트랙에 dynaudnorm 1회 — 라인 간 상대 음량만 평탄화.
    길이 불변이라 scene_timings 와 무관. assemble 의 integrated loudnorm 과 충돌 없음."""
    _run(["ffmpeg", "-y", "-i", str(src), "-af", _DYNAUDNORM,
          "-ar", "44100", "-ac", "1", "-c:a", "pcm_s16le", str(dst)])
...
def generate_tts(
    ...
    # 품질 플래그(전부 env, 기본값=현재 동작). 롤백 = env 원복/삭제 후 재배포.
    try:
        speed = float(os.getenv("SAYEON_TTS_SPEED") or "1.0")
    except ValueError:
        speed = 1.0
    speed = min(2.0, max(0.5, speed))  # atempo 단일단 유효 범위로 클램프
    volume_mode = (os.getenv("SAYEON_TTS_VOLUME_MODE") or "current").strip().lower()
    ...
    # 단일 내레이터(ELEVENLABS_VOICE_ID). 마지막 씬(질문 엔딩) 포함 전 라인을
    # 동일 내레이터가 읽는다(멀티보이스 보류 — 대사 구분은 자막 색상으로만).
    ...
    for n, (idx, text) in enumerate(narrations, 1):
        ...
        _to_wav(raw, wav, speed)  # speed!=1.0 이면 atempo 적용 → 아래 _duration 이 자동 정합
        dur = _duration(wav)
    ...
    if volume_mode == "even":
        # 라인 간 음량 평탄화(엔딩 작게/들쭉날쭉 개선). 길이 불변 → 타이밍 영향 없음.
        even = out_dir / "narration_even.wav"
        _even_loudness(combined, even)
        combined = even
