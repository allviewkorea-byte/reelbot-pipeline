...
def build_title(theme: dict, viz_spec: dict | None) -> str:
    """𝐏𝐥𝐚𝐲𝐥𝐢𝐬𝐭 | {감정 카피} {이모지} {장르 영문} {용도}. 100자 이내."""
    vs = viz_spec or {}
    copy = _copy(theme, vs)
    emoji = _emoji(theme, vs)
    gen = genre_en(theme, vs)
    purpose = "·".join(_purposes(theme, vs))
    title = f"{SIGNATURE} | {copy} {emoji} {gen} {purpose}".strip()
    if len(title) > 100:
        # 용도부터 줄여 100자 보장(시그니처·카피 보존).
        title = f"{SIGNATURE} | {copy} {emoji} {gen}".strip()[:100]
    return title
