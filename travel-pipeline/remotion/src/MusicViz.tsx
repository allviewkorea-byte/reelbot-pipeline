...
export type MusicVizProps = {
  ...
  bgUrl?: string;
  characterUrl?: string;
  muted?: boolean; // 청크 렌더 시 true → <Audio> 미렌더 + 이퀄 사인파 강제(오디오 메타 타임아웃 회피)
};
...
export const MusicViz: React.FC<MusicVizProps> = ({ tracks, mood, durationSec, vizSpec, designConfig, showPlaylist, hasCharacter, audioUrl, bgUrl, characterUrl, muted }) => {
  ...
  const rawAudioData = useAudioData(audioSrc);
  const audioData = muted ? null : rawAudioData;
