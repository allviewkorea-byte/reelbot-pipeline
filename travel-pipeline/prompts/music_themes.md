...
### 장르
시티팝 · 피아노 · 재즈 · 발라드 · 로파이(lo-fi) · 재즈힙합(jazz-hop) · 힙합 · R&B · K-R&B/한국 감성 · 소울/펑크 · 빈티지 소울/모타운 · EDM/하우스 · 신스웨이브 · 보사노바 · 어쿠스틱/포크 · 앰비언트 · 국악 퓨전 · 클래식 크로스오버 · K-pop · 팝(pop)
...
## 2-1. 재생목록 분류 — 14장르 (#45 SSOT)
유튜브에서는 모든 영상이 아래 **14개 재생목록 카테고리** 중 하나로 묶인다. 위 팔레트(장르×상황×무드)는
*생성 어휘*이고, 14장르는 *분류 버킷*이다 — 자유롭게 조합하되, 뽑은 주제가 아래 14개 중 하나로
자연스럽게 분류되도록 한다(분류는 시스템이 장르·상황·무드 텍스트로 자동 수행).

```
시티팝, 선셋 드라이브, 모닝 드라이브, 카페, 재즈,
발라드, 이별, 운동/동기부여, 수면/공부, 로파이,
K-pop, 팝송, R&B/소울, 힙합
```

- **드라이브**는 시간대로 갈린다: 아침·출근·상쾌 → *모닝 드라이브*, 노을·퇴근·몽환 → *선셋 드라이브*.
- **이별**은 상황(이별·쓸쓸·회상)으로, **발라드**는 장르로 — 둘 다 가능.
- 다양성 규칙(§5)은 이 14버킷에도 적용 — 한 버킷이 며칠씩 반복되지 않게 골고루 순환한다.
- 14버킷에 직접 안 들어가는 팔레트 장르(피아노·앰비언트·보사노바·국악 퓨전 등)는 상황·무드에 따라
  가장 가까운 버킷(예: 피아노+공부→수면/공부, 보사노바+카페→카페)으로 분류된다 — 팔레트는 그대로 쓴다.
