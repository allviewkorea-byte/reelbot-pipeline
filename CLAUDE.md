...
> 📌 where;_____ 분할 렌더 버그 전말·해결책: [WHERE_버그_인수인계_2026-06-28.md](./WHERE_버그_인수인계_2026-06-28.md)
...
2. **PR 번호를 보고한다.** 아래 "자율 머지 허용" 세 조건을 모두 만족하면 머지하고
   머지 사실까지 보고하고, 하나라도 어긋나면 **머지하지 않고 대기**한다.

### 자율 머지 허용 (지시 없이 `merge_pull_request` 가능)

아래 **세 조건을 모두** 만족할 때만 자율 머지한다.

1. **변경 범위가 아래 중 하나**
   - 주석·docstring·문서(md)만 변경
   - 테스트 파일만 추가
   - 이미 승인된 작업의 오타 수정
   - **프론트 전용 변경** — `src/` 아래만 바뀌고 `travel-pipeline/` 변경이 **0건**
     (Vercel 만 재배포되므로 진행 중인 렌더에 영향이 없다)

2. **SQL 선행이 필요 없다** — `docs/*.sql` 신규 파일이 없고, DB 스키마에
   의존하는 신규 컬럼·테이블 참조가 없다.

3. **CI 가 전부 통과**했고 `tsc` / `npm run build` / `py_compile` 이 exit 0 이다.

자율 머지했으면 **머지했다고 보고한다**(PR 번호·머지 사실·배포 대상).

### 반드시 검수 대기 (대표 지시 전 머지 금지)

- **`travel-pipeline/` 변경이 1건이라도 있는 PR** — Railway 재배포가 진행 중인
  렌더를 죽인다. 렌더 상태는 Claude Code 가 확인할 수 없으므로 반드시 대표가
  "렌더 유휴"를 확인한 뒤 지시한다.
- **SQL 선행이 필요한 PR** — 대표가 Supabase 에서 실행하고 **백필을 확인한 뒤**에만
  머지한다. 순서가 바뀌면 PostgREST 400 이 난다.
...
## 워크플로 패턴
claude.ai(전략·작업지시서) → Claude Code(코딩·PR) → 머지 → 자동배포 → 시크릿창 확인.
- 머지 주체는 변경 범위가 가른다: 프론트 전용·문서는 Claude Code 자율 머지(→ Vercel),
  `travel-pipeline/` 가 섞이면 대표가 렌더 유휴를 확인하고 지시(→ Railway).
  판단 기준은 위 "PR·브랜치·머지 규칙"의 두 블록이 유일한 정의다.
